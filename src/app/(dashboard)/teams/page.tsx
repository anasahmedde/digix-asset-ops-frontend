...
const btnSecondary =
  "inline-flex h-10 items-center rounded-lg border border-gray-200 bg-transparent px-4 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900";
const btnPrimary =
  "inline-flex h-10 items-center rounded-lg bg-gradient-to-r from-teal-500 to-teal-600 px-5 text-sm font-medium text-gray-900 shadow-lg shadow-teal-500/20 transition-all hover:shadow-teal-500/30 disabled:opacity-50";
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} className="px-5 py-10 text-center text-gray-400">
                    <div className="flex items-center justify-center gap-2">
                      <div className="h-4 w-4 animate-spin rounded-full border-2 border-teal-500/30 border-t-teal-500" />
                      Loading...
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={7} className="px-5 py-10 text-center text-gray-400">{users.length > 0 ? "No users match your filters" : "No users found"}</td>
                </tr>
              ) : (
                filtered.map((user) => (
                  <tr key={user.id} onClick={() => openEdit(user)} className="border-b border-gray-200 cursor-pointer transition-colors hover:bg-teal-50/40">
                    <td className={`${tdClass} font-medium text-gray-900`}>{user.full_name || "-"}</td>
                    <td className={`${tdClass} text-gray-600`}>{user.username}</td>
                    <td className={`${tdClass} text-gray-600`}>{user.email}</td>
                    <td className={tdClass}>
                      <span className="inline-flex rounded-full bg-teal-500/10 px-2.5 py-0.5 text-xs font-medium capitalize text-teal-600 ring-1 ring-teal-500/20">
                        {user.role.replace("_", " ")}
                      </span>
                    </td>
                    <td className={`${tdClass} text-gray-600`}>{user.is_field_staff ? "Yes" : "No"}</td>
                    <td className={tdClass}>
                    ...
                    <td className={tdClass} onClick={(e) => e.stopPropagation()}>
                      {isAdmin ? (
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEdit(user)} className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-900" title="Edit user">
                            <Pencil className="h-3.5 w-3.5" />
                          </button>
                          <button onClick={() => openResetPassword(user)} className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-500 transition-colors hover:bg-gray-100 hover:text-amber-400" title="Reset password">
              ...
              <form onSubmit={modalMode === "create" ? handleCreate : handleEdit} className="space-y-4">
                ...
                {modalMode === "edit" && (
                  <div className="space-y-1.5">
                    ...
                    <input type="text" value={selected?.username ?? ""} disabled className="flex h-10 w-full rounded-lg border border-gray-200 bg-gray-100 px-3 text-sm text-gray-400" />
                  </div>
                )}
                ...
                <div className="flex items-center gap-2">
                  <input id="is_field_staff" name="is_field_staff" type="checkbox" defaultChecked={selected?.is_field_staff ?? false} className="h-4 w-4 rounded border-gray-200 bg-white accent-teal-500" />
                  <label htmlFor="is_field_staff" className="text-sm text-gray-600">Field Staff (mobile app access)</label>
                </div>
